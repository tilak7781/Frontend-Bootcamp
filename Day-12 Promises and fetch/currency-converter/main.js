document.addEventListener("DOMContentLoaded", () => {
  document
    ...
    .addEventListener("submit", async (event) => {
      ...
      headers.append("apikey", "8r0HjFx8LOrXSS0Ol9FlK9oH2rnfOprK");
      ...
      fetch(
        ...
        .then((data) => {
          ...
          document.querySelector(
            ".result"
          ).textContent = `As per the exchange rate :${info.rate.toFixed(
            2
          )} for ${date} => converted value in ${to} is ${result.toFixed(2)} `;
        })
